...
while norm(R, 2) > epsilon
    ...
    j_v_approx = @(v)JV_APPROX(v, F, x);               % --Jv
        
    tol=1e-2;
    
    v = gmres(j_v_approx, R,[], tol);                          % solve for Krylov vector  -- du
    
    x = x - v;                                                          % updated solution -- Jv*du=R
    
    matx=vec2mat(x,n_e)';
    
    matR = F(matx);                                               % new residual
